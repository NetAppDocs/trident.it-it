...
.Fasi
. Se link:../trident-get-started/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Installato Trident utilizzando Helm"], puoi usare  `helm upgrade trident netapp-trident/trident-operator --version 100.2602.0` per aggiornare in un unico passaggio. Se non hai aggiunto il Helm repo o non puoi usarlo per aggiornare:
+
...
.. Usa il  `helm upgrade` comando dove  `trident-operator-26.02.0.tgz` riflette la versione a cui vuoi eseguire l'aggiornamento.
+
[listing]
----
