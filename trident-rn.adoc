...
== Novità della versione 26.06.1

Scopri le novità in NetApp Trident e Trident Protect, inclusi miglioramenti, correzioni e deprecazioni.
...
* *Kubernetes*:
+
** Risolto un problema di competizione tra la pubblicazione CSI e lo spostamento del volume che poteva collegare un volume utilizzando informazioni di accesso obsolete.
** Risolto un problema di competizione in cui NodeGetInfo poteva restituire etichette di topologia vuote prima del completamento della registrazione del nodo, causando errori di provisioning basati sulla topologia.
** Risolto un raro problema di condizione di gara nella disconnessione NVMe che poteva causare la chiusura di un sottosistema condiviso mentre un altro pod era ancora in fase di connessione.
** Risolti gli errori di pubblicazione dei cloni NVMe PVC (e la mappatura errata del namespace padre) durante la creazione di cloni di un volume montato in modalità concorrente.
** Risolti gli errori di creazione degli snapshot per i PVC sui backend ONTAP-SAN-Economy utilizzando un nameTemplate.
...
* *Kubernetes*:
+
** Aggiunta l'opzione `storageAPITimeout` (secondi) nella configurazione del backend ONTAP per impostare i timeout dei client API REST e ZAPI per ciascun backend.
...
* *Kubernetes*:
+
** Aggiunto il supporto generale (GA) per la concorrenza per i driver ONTAP-NAS-Economy e ONTAP-SAN-Economy, facendo uscire questa funzionalità dalla Tech Preview, e aggiunto il supporto GA per la concorrenza anche per i driver Azure NetApp Files (ANF) e SolidFire.
** Aggiunto il supporto per Kubernetes 1.36.
** Aggiunto il supporto generale (GA) per l'autogrow dei volumi con i driver NAS e SAN di Google Cloud NetApp Volumes (GCNV).
** Aggiunto il posizionamento bilanciato Unified ONTAP per una selezione ottimale degli aggregati durante il provisioning di nuovi volumi (link:https://github.com/NetApp/trident/issues/64["Issue #64"]).
** Aggiunto il supporto ONTAP AFX per il driver ONTAP-NAS-Flexgroup.
** Aggiunta l'integrazione con il NetApp Shift Toolkit per semplificare la migrazione delle macchine virtuali quando usi il Red Hat OpenShift Migration Toolkit for Virtualization.
** Aggiunto il supporto Cloud Identity per il driver SAN GCNV.
** Aggiunto il supporto per i volumi subordinati per la propagazione di Trident Volume Publication (TVP) e i flussi di lavoro di crescita automatica in Trident.
** Aggiunto il supporto per gli aggiornamenti dei volumi e gli snapshot dei gruppi di volumi quando usi Trident con la concorrenza abilitata.
** Aggiunta la limitazione dinamica della frequenza gRPC per GCNV.
** Aggiunto un probe di esecuzione per la verifica della prontezza del controller e ottimizzate le impostazioni predefinite di QPS e Burst dell'API Kubernetes per il controller e i sidecar.
** Sequenziamento delle operazioni sui nodi migliorato grazie alla separazione delle fasi di collegamento, configurazione della crittografia e formattazione del filesystem.
** I filesystem root dei container Trident sono stati montati in modalità di sola lettura per migliorare la sicurezza.
** Aggiunta la rimozione best-effort degli snapshot dei volumi clone nei backend ONTAP quando viene eliminato lo snapshot di origine di un volume clone.
** Aggiunto il riconoscimento dei PVC di clonazione di backup per impedire che i PVC temporanei creati da Veeam Kasten, Trilio e Cohesity rimangano bloccati nella ONTAP Volume Recovery Queue.
** Gestione avanzata degli ARN AWS per il supporto di regioni speciali.
** Impedisci la riduzione delle dimensioni dei Flexvol durante l'operazione di ridimensionamento per il driver ONTAP-SAN-ECONOMY.
...
* *[Anteprima tecnica]*: Aggiunto il supporto per l'importazione di qtree ONTAP-NAS-Economy.
* *[Anteprima tecnica]*: Aggiunto il supporto per la concorrenza per i driver ASA-R2 (SAN e NVMe).
* *[Anteprima tecnica]*: Aggiunto trasporto HTTP GCNV in modalità ONTAP per i driver ONTAP-SAN e ONTAP-NAS.
...
* *Kubernetes*:
+
** Risolti i problemi di race condition e la gestione dello stato per le operazioni simultanee di pubblicazione/annullamento pubblicazione, clonazione, cache e aggiornamento del backend.
** Risolte le condizioni di competizione delle policy di esportazione e la gestione simultanea di pubblicazione/annullamento pubblicazione per i volumi subordinati e i cloni di sola lettura nei driver ONTAP-NAS e ONTAP-NAS-Economy.
** Corretto il rilevamento ONTAP-NAS-Economy FlexVol dopo un failover MetroCluster quando le policy di snapshot differiscono (link:https://github.com/NetApp/trident/issues/1082["Issue #1082"]).
** Risolti i tentativi di eliminazione del volume ONTAP-NAS-Economy che interrompevano le eliminazioni di qtree di lunga durata (link:https://github.com/NetApp/trident/issues/1121["Issue #1121"]).
** Risolto il comportamento di importazione e ridimensionamento di ONTAP-SAN e ONTAP-SAN-Economy, inclusa la convalida di fsType, la gestione dei metadati del volume e il comportamento della modalità di autogrow.
** Risolto un problema di race condition nello spazio dei nomi NVMe/TCP durante la creazione simultanea di volumi (link:https://github.com/NetApp/trident/issues/1089["Issue #1089"]).
** Risolti gli errori di ridimensionamento iSCSI che si verificavano quando i percorsi e i portali disponibili non corrispondevano e migliorato il comportamento dei ridimensionamenti ripetuti.
** Risolti gli errori di eliminazione dei volumi ONTAP-SAN causati da condizioni di volume occupato.
** Corretto la gestione degli errori ONTAP REST e migliorato il comportamento del timeout dello stato SVM.
** Gestione corretta del pool zonale SAN GCNV e del gruppo host, conversione normalizzata dell'identificativo del volume e normalizzazione delle richieste di creazione al di sotto della dimensione minima.
** Corretta la selezione del pool di capacità nativo GCNV per escludere i pool in modalità ONTAP.
** Risolto il failover di KubeVirt per le VM che utilizzano Trident Automated Workload Failover per velocizzare il failover delle VM.
** Risolti i problemi RBAC delle installazioni gestite dall'operatore, inclusi i ClusterRoles dei nodi mancanti e la riconciliazione errata di ClusterRole e ClusterRoleBinding durante gli aggiornamenti. Questo vale per le installazioni basate su Operator e Helm che utilizzano il chart Trident Operator.
** Risolti diff persistenti di GitOps su TridentConfigurator e CRD correlati omettendo i valori di priorità predefiniti e correggendo l'indentazione del template YAML (link:https://github.com/NetApp/trident/issues/915["Issue #915"], link:https://github.com/NetApp/trident/issues/1120["Issue #1120"]).
** Corretto il patching dei CRD TridentVolumePublication durante un upgrade di Helm (link:https://github.com/NetApp/trident/issues/1151["Issue #1151"]).
** Corretto l'ordine di avvio di CSI avviando il servizio CSI gRPC prima dei tentativi di registrazione del nodo.
** Risolta la sovrapposizione della configurazione di clonazione SMB sicura durante le operazioni simultanee.
** Corrette le annotazioni mancanti sul pod controller Trident dopo un aggiornamento (link:https://github.com/NetApp/trident/issues/1004["Issue #1004"]).
** Provisioning di qtree lenti migliorato su SVM di grandi dimensioni (link:https://github.com/NetApp/trident/issues/1142["Issue #1142"]).
** Corretto ControllerPublish per usare il tipo di filesystem di configurazione del volume invece di un tipo di filesystem predefinito errato.
** Risolto un problema per cui i dispositivi dm orfani potevano causare ripetuti errori CSI NodeUnstageVolume.
** Pulizia migliorata dei nodi obsoleti e dei percorsi multipli, rimuovendo gli artefatti fantasma dei percorsi multipli e le risorse TridentNode obsolete dopo la pulizia dei nodi eliminati in modo soft.
...
* Aggiunta l'integrazione con OpenShift Service CA Operator per automatizzare la gestione dei certificati TLS per il webhook di ammissione di Trident Protect. Abilita impostando  `useOpenshiftServiceCA` su  `true` nei tuoi valori Helm. Per saperne di più, consulta link:trident-protect/trident-protect-customize-installation.html["Personalizza l'installazione Trident Protect"^].
* Aggiunto il supporto per il backup e il ripristino delle VM KubeVirt su OpenShift Virtualization. Proteggi tutte le VM in un namespace oppure seleziona VM specifiche per nome e ripristina singole VM da un backup o da uno snapshot. Per saperne di più, consulta link:trident-protect/trident-protect-kubevirt-vms.html["Proteggi le macchine virtuali KubeVirt con Trident Protect"^].
* Aggiunto il supporto per le trasformazioni delle risorse, che ti permette di aggiungere, rimuovere e modificare gli attributi delle risorse Kubernetes durante il processo di ripristino. Per saperne di più, consulta link:trident-protect/trident-protect-resource-transformations.html["Modifica le risorse durante il ripristino utilizzando risorse personalizzate"].
* Aggiunto il supporto per  `storageClassMapping` in AppMirrorRelationship (AMR), che consente la mappatura delle classi di archiviazione per i singoli PVC all'interno di una singola definizione AMR. Per saperne di più, consulta link:trident-protect/trident-protect-use-snapmirror-replication.html["Replicare le applicazioni utilizzando NetApp SnapMirror e Trident Protect"^].
...
* Risolto il problema per cui il ripristino di macchine virtuali tra cluster non riusciva a causa di un errore del webhook di ammissione quando lo spazio dei nomi dell'applicazione di origine non esisteva nel cluster di destinazione.
* Risolto il problema per cui i ripristini selettivi in-place delle macchine virtuali non riuscivano nella fase di creazione dell'applicazione a causa di una mancata corrispondenza delle specifiche con una risorsa personalizzata dell'applicazione esistente; ora l'applicazione esistente viene riutilizzata.
* Risolto il problema per cui i processi di rispristino in-place delle macchine virtuali si bloccavano in uno stato  `Running` quando sia la macchina virtuale che il suo PersistentVolumeClaim venivano eliminati; un PersistentVolumeClaim mancante viene ora considerato come un errore irreversibile.
* Risolto il problema che causava l'attivazione errata di un backup o di uno snapshot immediato durante la modifica di una pianificazione di protezione esistente.
* Corretto il finalizzatore dello snapshot che tentava di recuperare la risorsa personalizzata dell'applicazione dopo che era stata eliminata; ora il finalizzatore salta quando l'applicazione non viene trovata.
* La capacità di archiviazione protetta fissa aumenta significativamente dopo un ripristino in-place di una macchina virtuale grazie all'utilizzo della copia sparsa durante la fase di copia di PersistentVolumeClaim.
* Risolto il problema per cui Trident Protect eliminava l'oggetto snapshot di Kubernetes prima di confermare l'eliminazione dello snapshot sottostante in ONTAP; ora il finalizzatore attende l'eliminazione di  `VolumeSnapshot`.
* Risolto il problema per cui gli snapshot non venivano eseguiti quando un'operazione di freeze di una macchina virtuale non riusciva; ora le operazioni di freeze e unfreeze continuano anche in caso di errore.
* Risolto il problema per cui le applicazioni delle macchine virtuali KubeVirt ricreavano in modo errato i PersistentVolumeClaims durante le operazioni di reverse, reverse resync e resync di AppMirrorRelationship.




[[whats-new-in-26-02-1]]
== Novità della versione 26.02.1
...
* *Kubernetes*:
* Corretto il comportamento SAN di Google Cloud NetApp Volumes (GCNV) per supportare i pool Flex Unified ZONAL per carichi di lavoro a blocchi (iSCSI). Sono ora supportate sia le configurazioni zonali che regionali.
* Corretta la gestione dei pool NAS UNIFIED di Google Cloud NetApp Volumes (GCNV) migliorando la creazione, la convalida, la denominazione e le operazioni di ricerca dei volumi di fallback quando  `InternalID` è assente.
* Risolto un problema per cui le operazioni di ridimensionamento dei dispositivi iSCSI potevano non riuscire quando i percorsi e i portali disponibili non corrispondevano.
* Risolto un problema intermittente che  `blkid` identificava in modo errato il tipo di filesystem per i dispositivi LUKS, impedendo la creazione di nuovi volumi.
* È stato risolto un problema per cui le operazioni di pubblicazione e staging di LUKS su larga scala potevano bloccare il controller Trident. Gli utenti sono ora tenuti a tenere traccia delle passphrase LUKS associate ai volumi.
* Risolto un problema per cui le partizioni iSCSI multipath potevano apparire come dispositivi fantasma, impedendo le operazioni CSI `NodeUnstage` di avere successo.
* Risolto un problema di race condition nello spazio dei nomi NVMe/TCP durante la creazione simultanea di volumi (Issue #1089).
* Risolto il problema delle operazioni di clonazione simultanee degli snapshot quando questi venivano eseguiti nello stesso secondo.
* Risolto un problema che impediva alle VM KubeVirt che utilizzano `dataVolumes` di eseguire il failover con Trident Automated Workload Failover.
* Risolto un errore che si verificava durante l'eliminazione di una risorsa personalizzata non valida  `TridentMirrorRelationship` mentre si utilizzava il core concorrente di Trident.
* Risolto il problema di indentazione nella `tridentactionmirrorupdates` definizione YAML del CRD di Kubernetes (Issue #1120).
* Risolto il problema relativo alla chiamata API REST per l'eliminazione asincrona di qtree (Issue #1121).
* Corretto `ControllerPublish` per utilizzare il tipo di file system di configurazione del volume anziché un tipo di file system predefinito errato.
* Risolto il problema di registrazione all'avvio del controller per segnalare correttamente lo stato dei driver concorrenti.
* Gestione corretta dei backend sospesi in Trident concorrente.
* Gestione avanzata degli ARN AWS per il supporto di regioni speciali.
...
Aggiornate le dipendenze di Trident Protect per risolvere le vulnerabilità di sicurezza:
...
* Runtime Go: link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* Dipendenze aggiuntive: link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]




[[whats-new-in-26-02]]
== Novità della versione 26.02
...
* *Kubernetes*:
+
** Aggiunto il supporto per la disponibilità generale (GA) per la concorrenza per ONTAP-NAS (solo NFS), ONTAP-SAN (iSCSI, FCP, NVMe) e Google Cloud NetApp Volumes (GCNV), promuovendo questa funzionalità dalla Tech Preview. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["Documentazione sulla scalabilità del controller"] per ulteriori informazioni.
** Aggiunto il supporto per l'autogrow del volume in Trident in base a criteri AutoGrow personalizzati e definiti dall'utente in Trident. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["Documentazione sull'espansione automatica del volume"] per ulteriori informazioni.
** Concorrenza dei nodi Trident migliorata per migliorare la scalabilità delle operazioni dei nodi per i volumi NVMe. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["Documentazione sui volumi NVMe"] per ulteriori informazioni.
** Aggiunto supporto per l'auto-tiering dei volumi NAS GCNV tramite annotazioni PVC (`tieringPolicy`, `tieringMinimumCoolingDays`, inclusa la selezione del pool e l'eredità dei cloni. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["Configura il tiering automatico per la documentazione di Google Cloud NetApp Volumes"] per ulteriori informazioni.
** Aggiunto supporto per volumi GCNV SAN block (iSCSI) utilizzando il  `google-cloud-netapp-volumes-san` driver, inclusi provisioning, mappatura dei gruppi host per nodo per l'accesso LUN e operazioni di clonazione da volume. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Documentazione sulla configurazione dei blocchi di Google Cloud NetApp Volumes"] per ulteriori informazioni.
** Aggiunto il supporto per la configurazione automatica del backend per Amazon FSx for NetApp ONTAP. Quando si crea un StorageClass che include i parametri richiesti, Trident crea automaticamente il backend corrispondente e VolumeSnapshotClass, se necessario. Vedere il link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAP documentazione sulla configurazione del backend"] per ulteriori informazioni.
** Aggiunto il supporto per diversi cloud Microsoft Azure, come Azure Government e Azure China, e la configurazione cloud personalizzata per i backend di Azure NetApp Files. Consultare la link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Documentazione sulla configurazione del backend di Azure NetApp Files"] per ulteriori informazioni.
** Aggiunto il supporto per Kubernetes 1.35. Consultare la link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["Documentazione dei requisiti"] per ulteriori informazioni.
...
* *[Anteprima tecnica]*: Aggiunto supporto per la concorrenza per i driver ONTAP-NAS-Economy e ONTAP-SAN-Economy.
...
* *Kubernetes*:
+
** Risolto un problema per cui l'annullamento della pubblicazione di un clone di sola lettura rimuoveva le regole della policy di esportazione dal volume di origine nei driver ONTAP-NAS, ONTAP-NAS-Economy e Google Cloud NetApp Volumes link:https://github.com/NetApp/trident/issues/1086["Issue #1086"].
** Sono state sostituite le immagini kubectl con varianti leggere basate su Alpine per evitare errori di pull in seguito alla deprecazione delle immagini pubbliche di Bitnami https://github.com/NetApp/trident/issues/1080["Issue #1080"].
** Corretto il problema di conservazione dell'annotazione della distribuzione esistente durante l'aggiornamento di Trident link:https://github.com/NetApp/trident/issues/1004["Issue #1004"].
** Consenti la clonazione tra diverse classi di archiviazione se entrambe le classi di archiviazione puntano allo stesso backend link:https://github.com/NetApp/trident/issues/1104["Issue #1104"].
** Risolti i problemi di preparazione dei nodi causati da timeout in ambienti cloud con latenze di rete. Aumentati i valori di timeout per le installazioni basate sul cloud.
** È stato risolto un problema nella creazione di LUN che causava la mancata impostazione dell'attributo del tipo di filesystem quando il processo entrava in uno stato di retry.
** Corretto il lookup del volume dell'API REST per ignorare lo stato del volume, prevenendo falsi negativi durante le query del volume.
** Migliorata l'efficienza del controller Trident per il driver ontap-nas-economy quando utilizzato su larga scala.
** Imposta internalID durante l'importazione LUN nel driver ontap-san-economy.
** Aumentati i limiti delle query di Azure Resource Graph per gestire più subnet.
** Migliorati i timeout di clone split di CSI e ONTAP per evitare condizioni di competizione con alcune applicazioni di backup link:https://github.com/NetApp/trident/issues/1098["Issue #1098"], link:https://github.com/NetApp/trident/issues/1100["Issue #1100"].
** Corretta la soppressione dei messaggi di errore LUKS link:https://github.com/NetApp/trident/issues/1069["Issue #1069"].
** Corretta la gestione dei mapper LUKS obsoleti per i protocolli iSCSI e NVMe. La logica di pulizia avanzata impedisce errori di montaggio causati da mapper di dispositivi orfani.


* Corrette le limitazioni di scalabilità per i volumi RWX NVMe.
* Aggiornato il pacchetto OpenTelemetry-Go per correggere link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"].
...
* Trident Protect ora disabilita automaticamente le pianificazioni di protezione e annulla le operazioni in corso prima di un rispristino in-place, e le riattiva dopo il completamento del ripristino. Per ulteriori informazioni, consultare link:trident-protect/trident-protect-restore-apps.html["Ripristina le applicazioni utilizzando Trident Protect"^].
* Aggiunto il campo  `runImmediately` al CR della pianificazione e il flag CLI  `--run-immediately` per attivare un backup o uno snapshot immediato al momento della creazione della pianificazione. Per ulteriori informazioni, consultare link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["Creare un piano di data Protection"^].
* Aggiunto il supporto per specificare un nome personalizzato per l'applicazione ripristinata utilizzando il  `destinationApplicationName` field nella restore CR o il  `--destination-app-name` CLI flag. Per ulteriori informazioni, fare riferimento a link:trident-protect/trident-protect-restore-apps.html["Ripristina le applicazioni utilizzando Trident Protect"^].
...
* Risolti i problemi di ripristino causati dalla creazione di pod prima che i relativi account di servizio richiesti fossero disponibili.
* Corretto  `Roles` e  `RoleBindings` che venivano saltati durante i ripristini delle applicazioni.
* Risolto il problema per cui il nome del cluster di origine non veniva visualizzato nell'output  `tridentctl-protect get appvaultcontent` nonostante fosse configurato correttamente.
* Corretti gli errori di ripristino di Kopia che venivano ignorati a causa della mancata gestione  `pipefail`.
* Risolti errori di snapshot e backup causati dai filtri delle risorse che escludevano i volumi persistenti.
* Risolto il problema del ripristino PVC errato nelle applicazioni multi-namespace con PVC con nomi identici tra namespace, che poteva causare la perdita di dati.
...
** Aggiunto supporto per il failover automatico del carico di lavoro con distacco forzato del volume per ONTAP-NAS e ONTAP-NAS-Economy (escluso SMB in entrambi i driver NAS) e per i driver ONTAP-SAN e ONTAP-SAN-Economy. Vederelink:https://docs.netapp.com/us-en/trident/trident-reco/force-detach.html["Automazione del failover delle applicazioni stateful con Trident"^] .
...
** Aggiunta un'opzione per fornire le metriche Prometheus con HTTPS, anziché HTTP. Vederelink:https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html["Monitor Trident"^] .
** Aggiunta un'opzione `--no-rename` quando si importa un volume per mantenere il nome originale ma lasciare che Trident gestisca il ciclo di vita del volume. Vederelink:https://docs.netapp.com/us-en/trident/trident-use/vol-import.html["Importa volumi"^] .
...
+
Vederelink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["Annotazioni di backup e pianificazione supportate"^] .



* Aggiunta un'annotazione alla pianificazione CR per configurare il timeout di associazione PVC, che verrà utilizzato dalla CR di backup: `protect.trident.netapp.io/pvc-bind-timeout-sec` . Vederelink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["Annotazioni di backup e pianificazione supportate"^] .
...
IMPORTANT: Per i clienti che utilizzano SolidFire, non aggiornare a 25.06.1 a causa di un problema noto durante l'annullamento della pubblicazione dei volumi. 25.06.2 verrà rilasciata a breve per risolvere questo problema.
...
** Risolto il problema per cui i volumi GCNV utilizzavano NFSv3 come predefinito quando  `nfsMountOptions` non era impostato; ora sono supportati entrambi i protocolli NFSv3 e NFSv4. Se  `nfsMountOptions` non viene fornito, viene utilizzata la versione NFS predefinita dell’host (NFSv3 o NFSv4).
