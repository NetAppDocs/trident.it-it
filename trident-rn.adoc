...
* Il supporto della concorrenza per i driver ONTAP-NAS (solo NFS), ONTAP-SAN (iSCSI, FCP, NVMe) e Google Cloud NetApp Volumes è ora generalmente disponibile (GA).
* Aggiunto il supporto per la configurazione automatica del backend per Amazon FSx for NetApp ONTAP. Quando si crea un StorageClass che include i parametri richiesti, NetApp Trident crea automaticamente il backend corrispondente e la VolumeSnapshotClass (se necessario).
* Aggiunto il supporto per diversi cloud Microsoft Azure, come Azure Government e Azure China, e la configurazione cloud personalizzata per i backend di Azure NetApp Files.
* Scalabilità del controller migliorata per i volumi NVMe.
* Aggiunto il supporto per la funzionalità di autogrow del volume. È possibile controllare il comportamento dell'autogrow definendo policy di autogrow personalizzate di NetApp Trident.
* Aggiunto supporto per Google Cloud NetApp Volumes storage a blocchi (SAN/iSCSI).
* Aggiunto il supporto per il tiering automatico per Google Cloud NetApp Volumes. Ora puoi ottimizzare i costi di archiviazione aggiungendo controlli nativi di Kubernetes per configurare il tiering automatico FabricPool di ONTAP per Google Cloud NetApp Volumes tramite NetApp Trident.
* Aggiunto il supporto per Kubernetes 1.35.
