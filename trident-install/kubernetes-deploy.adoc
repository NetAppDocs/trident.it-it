...
[]
=====
* Kubernetes ora è supportato in Trident. Aggiorna Trident prima di aggiornare Kubernetes.
...
Che si utilizzi la distribuzione manuale o Helm, l'operatore Trident è un ottimo modo per semplificare l'installazione e gestire dinamicamente le risorse Trident. Puoi anche link:kubernetes-customize-deploy.html["Personalizzare l'implementazione dell'operatore Trident"] utilizzando gli attributi nella  `TridentOrchestrator` custom resource (CR).
