...
NOTE: Per definire un'applicazione con ambito VM che protegge specifiche KubeVirt macchine virtuali (VM) per nome anziché interi namespace, usa il campo  `spec.includedVirtualMachines` nell'Application CR o il flag  `--virtual-machines` nella CLI. Per dettagli, riferimenti ai campi ed esempi YAML, consulta link:trident-protect-kubevirt-vms.html["Proteggi le macchine virtuali KubeVirt con Trident Protect"].
...
. (_Facoltativo_) Se necessario, puoi aggiungere il filtro delle risorse alla stessa CR per includere o escludere risorse specifiche:
+
** *Esempio di filtro generico:*
+
...
** *Esempio di filtro solo PVC:*
+
Per definire un'applicazione solo PVC, è necessario includere anche  `PersistentVolume` e  `VolumeSnapshotClass` nel filtro delle risorse. Le operazioni di Snapshot e backup dipendono da  `PersistentVolume` (il volume con ambito cluster associato a ciascun PVC) e  `VolumeSnapshotClass` (il driver di snapshot), e non funzioneranno senza di essi. Ad esempio:
...
+ Per specificare l'annotazione quando usi la CLI per creare un'applicazione, puoi usare il  `--annotation` flag.

+ * Crea l'applicazione e utilizza l'impostazione globale per il comportamento di blocco del filesystem:
...
----
+ * Crea l'applicazione e configura le impostazioni locali dell'applicazione per il comportamento di blocco del filesystem:
...
----
+ * Per definire un'applicazione solo PVC, devi includere anche  `PersistentVolume` e  `VolumeSnapshotClass`` nel filtro delle risorse. Le operazioni di Snapshot e backup dipendono da  `PersistentVolume` (il volume con ambito cluster associato a ciascun PVC) e  `VolumeSnapshotClass` (il driver di Snapshot), e non funzioneranno senza di essi. Per esempio:
