...
* Per ripristinare specifiche macchine virtuali (VM) KubeVirt da un archivio senza ripristinare tutte le risorse dello spazio dei nomi, usa il campo  `includedVirtualMachines` su una CR di ripristino. Per le CR supportate, il riferimento ai campi e gli esempi YAML, consulta link:trident-protect-kubevirt-vms.html["Proteggi le macchine virtuali KubeVirt con Trident Protect"].
...
** *spec.destinationApplicationName*: (_Facoltativo_) Il nome per l'applicazione ripristinata. Se specificato, l'applicazione ripristinata utilizza questo nome. Se non specificato, l'applicazione ripristinata utilizza il nome dell'applicazione di origine.
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ . (_Facoltativo_) Se hai bisogno di selezionare solo determinate risorse dell'applicazione da ripristinare, aggiungi un filtro che includa o escluda le risorse contrassegnate con etichette specifiche:

+ NOTA: Trident Protect seleziona automaticamente alcune risorse in base alla loro relazione con le risorse che selezioni. Ad esempio, se selezioni una risorsa di tipo persistent volume claim e questa ha un pod associato, Trident Protect ripristinerà anche il pod associato.

+ * *resourceFilter.resourceSelectionCriteria*: (Obbligatorio per il filtraggio) Usa  `Include` o  `Exclude` per includere o escludere una risorsa definita in resourceMatchers. Aggiungi i seguenti parametri resourceMatchers per definire le risorse da includere o escludere: ** *resourceFilter.resourceMatchers*: Un array di oggetti resourceMatcher. Se definisci più elementi in questo array, corrispondono come un'operazione OR e i campi all'interno di ciascun elemento (group, kind, version) corrispondono come un'operazione AND. *** *resourceMatchers[].group*: (_Opzionale_) Gruppo della risorsa da filtrare. *** *resourceMatchers[].kind*: (_Opzionale_) Tipo della risorsa da filtrare. *** *resourceMatchers[].version*: (_Opzionale_) Versione della risorsa da filtrare. *** *resourceMatchers[].names*: (_Opzionale_) Nomi nel campo metadata.name di Kubernetes della risorsa da filtrare. *** *resourceMatchers[].namespaces*: (_Opzionale_) Namespace nel campo metadata.name di Kubernetes della risorsa da filtrare. *** *resourceMatchers[].labelSelectors*: (_Opzionale_) Stringa del selettore di etichette nel campo metadata.name di Kubernetes della risorsa come definito in  https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Documentazione Kubernetes"^]. Ad esempio:  `"trident.netapp.io/os=linux"`.
  ...
  resourceFilter:
    ...
    resourceMatchers:
      ...
      - group: my-resource-group-2
        ...
        labelSelectors: ["trident.netapp.io/os=linux"]
----
+ . Dopo aver inserito i valori corretti nel file  `trident-protect-snapshot-ipr-cr.yaml`, applica il CR:
