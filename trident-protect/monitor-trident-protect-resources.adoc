...
.Fasi
. Integra metriche-stato-kube con Prometheus. Modificare il file di configurazione di Prometheus (`prometheus.yml`) e aggiungere le informazioni del servizio kube-state-metrics. Ad esempio:
+
.prometheus.yml: Integrazione del servizio kube-state-metrics con Prometheus
[source, yaml]
----
---
apiVersion: v1
...
----
. Configurare Prometheus per instradare gli avvisi ad Alertmanager. Modificare il file di configurazione di Prometheus (`prometheus.yml`) e aggiungere la seguente sezione:
+
.prometheus.yml: Invia avvisi ad Alertmanager
[source, yaml]
----
alerting:
  alertmanagers:
...
Nell'esempio seguente viene definito un avviso critico che viene attivato quando lo stato della risorsa personalizzata di backup è impostato su `Error` per 5 secondi o più. È possibile personalizzare questo esempio in base all'ambiente in uso e includere questo frammento YAML nel `prometheus.yml` file di configurazione:

.Rules.yml: Definire un avviso Prometheus per i backup non riusciti
[source, yaml]
----
rules.yml: |
  groups:
...
È possibile configurare Alertmanager in modo che invii notifiche ad altri canali, quali e-mail, PagerDuty, Microsoft Teams o altri servizi di notifica specificando la rispettiva configurazione nel `alertmanager.yml` file.
...
.Alertmanager.yml: Invia avvisi a un canale Slack
[source, yaml]
----
data:
  alertmanager.yml: |
