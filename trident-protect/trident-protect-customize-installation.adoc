...
Puoi personalizzare le impostazioni di AutoSupport, il filtro dei namespace e la gestione dei certificati webhook di OpenShift per soddisfare le tue esigenze specifiche. La tabella seguente descrive i parametri di configurazione disponibili:
...
| useOpenshiftServiceCA | booleano | Consente l'integrazione con l'operatore CA di servizio nativo di OpenShift per la creazione e l'iniezione automatica dei certificati TLS per il webhook di ammissione di Trident Protect. Se impostato su  `true`, l'operatore CA di servizio gestisce automaticamente i certificati TLS del webhook. Usa solo su cluster OpenShift. (predefinito:  `false`) 
|===
