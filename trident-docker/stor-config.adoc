...
Opzione NAS per l'accesso alla `.snapshot` directory.
 a| 
Consente o disabilita l'accesso alla  `.snapshot` directory.

Valori validi: `true`, `false`. Il comportamento predefinito potrebbe variare in base alla versione di ONTAP e Trident. Impostare esplicitamente per garantire un comportamento coerente.
...
Per informazioni sui FlexGroups e sui carichi di lavoro appropriati per i FlexGroups fare riferimento a  https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf["Guida alle Best practice e all'implementazione del volume NetApp FlexGroup"^].
