...
----
+
** A partire dalla versione 25,02, Trident supporta la configurazione backend automatica. Trident crea le classi di backend e storage senza problemi dopo l'installazione di Trident. Per abilitare la configurazione automatica del backend, aggiungere il `ontapConfigurator` parametro e specificare `authType`, `fsxnID` `protocols` durante l'installazione.
+
[listing]
----
