...
È possibile eseguire il provisioning dei volumi SMB utilizzando `ontap-nas` autista. Prima di completare <<Integrazione dei driver ONTAP SAN e NAS>> completa questi passaggi: link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Preparatevi al provisioning dei volumi SMB"].
