...
I passaggi descritti in questa sezione illustrano come creare il backend separatamente. Per informazioni su come utilizzare la funzione Backend automatico, consultare la sezione link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["Configurare StorageClass e PVC"] per ulteriori informazioni.
...
È possibile eseguire il provisioning dei volumi SMB utilizzando il `ontap-nas` driver. Prima di completare link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["Integrazione dei driver ONTAP SAN e NAS"] completare questi passaggi: link:worker-node-prep.html#prepare-to-provision-smb-volumes["Preparatevi al provisioning dei volumi SMB"].
