...
Prima di poter eseguire il provisioning dei volumi tramite Trident, è necessario integrare il backend di storage con Trident creando una Trident Backend Configuration (TBC). La TBC definisce come Trident si connette al backend di storage e fornisce le informazioni necessarie a Trident per gestire le risorse di storage.



== Crea una configurazione backend Trident (TBC) per FSx for ONTAP

Esistono due modi per configurare il backend Trident per utilizzare un file system FSx for ONTAP: * Creare un StorageClass che sfrutti la funzionalità Automatic Backend. * Creare il backend e il StorageClass separatamente.

I passaggi descritti in questa sezione illustrano come creare il backend separatamente. Per informazioni su come utilizzare la funzione Backend automatico, consultare la sezione link:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap["Configurare StorageClass e PVC"] per ulteriori informazioni.
...
È possibile eseguire il provisioning dei volumi SMB utilizzando il `ontap-nas` driver. Prima di completare link:trident-fsx-storage-backend.html#ontap-san-and-nas-driver-integration["Integrazione dei driver ONTAP SAN e NAS"] completare questi passaggi: link:worker-node-prep.html#prepare-to-provision-smb-volumes.html["Preparatevi al provisioning dei volumi SMB"].
