...
NOTE: Se utilizzi credenziali "vsadmin", `managementLIF` devono essere quelle della SVM; se utilizzi credenziali "admin", `managementLIF` devono essere quelle del cluster.
...
`useREST` Quando è impostato su `true`, Trident utilizza le API REST ONTAP per comunicare con il backend; quando è impostato su `false`, Trident utilizza le chiamate ONTAPI (ZAPI) per comunicare con il backend. Questa funzione richiede ONTAP 9.11.1 e versioni successive. Inoltre, il ruolo di accesso ONTAP utilizzato deve avere accesso all' `ontap` applicazione. Ciò è soddisfatto dai ruoli predefiniti `vsadmin` e `cluster-admin` . A partire da Trident 24,06 e ONTAP 9.15.1 o versioni successive, `userREST` è impostato su `true` per impostazione predefinita; passare
...
La crittografia LUKS non è supportata per NVMe/TCP. | "" impostato su `false` per ASA R2. 
