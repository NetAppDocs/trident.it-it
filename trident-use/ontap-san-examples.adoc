...
[role="lead"]
Crea e usa i driver SAN ONTAP con la tua installazione di Trident. Questa sezione fornisce esempi di configurazione del backend e dettagli per la mappatura dei backend a StorageClasses. link:https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["Sistemi ASA r2"^] differiscono dagli altri sistemi ONTAP (ASA, AFF e FAS) nell'implementazione del loro livello di storage. Queste variazioni influiscono sull'uso di alcuni parametri come indicato. link:https://docs.netapp.com/us-en/asa-r2/learn-more/hardware-comparison.html["Scopri di più sulle differenze tra i sistemi ASA r2 e gli altri sistemi ONTAP"^]. Nella configurazione del backend di Trident, non devi specificare che il tuo sistema è ASA r2. Quando selezioni  `ontap-san` come  `storageDriverName`, Trident rileva automaticamente ASA r2 o altri sistemi ONTAP. Alcuni parametri di configurazione del backend non sono applicabili ai sistemi ASA r2 come indicato nella tabella seguente.
...
A partire dalla versione 26.06, puoi abilitare il posizionamento bilanciato di ONTAP così che ONTAP selezioni l'aggregato per i nuovi volumi. Consulta link:ontap-balanced-placement.html["Usa un posizionamento bilanciato con i backend ONTAP"] per i requisiti e gli esempi.
...
| `storagePrefix` | Prefisso utilizzato durante il provisioning di nuovi volumi nell'SVM. Non può essere modificato in seguito. Per aggiornare questo parametro, devi creare un nuovo backend. | `trident` 
...
NOTE: Quando l'aggregato viene aggiornato in SVM, viene aggiornato automaticamente anche in Trident tramite polling di SVM, senza dover riavviare il Trident Controller. Quando hai configurato un aggregato specifico in Trident per il provisioning dei volumi, se l'aggregato viene rinominato o spostato fuori da SVM, il backend passa allo stato di errore in Trident durante il polling dell'aggregato SVM. Devi cambiare l'aggregato con uno presente su SVM oppure rimuoverlo del tutto per riportare online il backend.
...
| `formatOptions`  a| 
Usa  `formatOptions` per specificare gli argomenti della riga di comando per il comando  `mkfs`, che viene applicato ogni volta che un volume viene formattato. Questo ti permette di formattare il volume secondo le tue preferenze. Assicurati di specificare le formatOptions in modo simile alle opzioni del comando mkfs, escludendo il percorso del dispositivo. Esempio: "-E nodiscard"
...
NOTE: Per tutti i volumi creati utilizzando il  `ontap-san` driver, Trident aggiunge un ulteriore 10% di capacità al FlexVol per ospitare i metadati del LUN. Il LUN viene predisposto con la dimensione esatta richiesta dall'utente nel PVC. Trident aggiunge il 10% al FlexVol (mostrato come Dimensione disponibile in ONTAP). Ora ottieni la quantità di capacità utilizzabile che hai richiesto. Questa modifica impedisce anche che i LUN diventino di sola lettura, a meno che lo spazio disponibile non sia completamente utilizzato. Questo non si applica a ontap-san-economy.
...
Le seguenti StorageClass definizioni si riferiscono a <<Esempi di backend con pool virtuali>>. Utilizzando il campo  `parameters.selector`, ciascuna StorageClass indica quali pool virtuali puoi usare per ospitare un volume. Ora il volume ha le caratteristiche definite nel pool virtuale scelto.

* Il `protection-gold` StorageClass mappa il primo pool virtuale nel `ontap-san` backend. Questo è l'unico pool che offre protezione di livello oro.
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* La  `protection-not-gold` StorageClass corrisponde al secondo e al terzo pool virtuale nel  `ontap-san` backend. Questi sono gli unici pool che offrono un livello di protezione diverso da gold.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* La `app-mysqldb` StorageClass corrisponde al terzo pool virtuale nel `ontap-san-economy` backend. Questo è l'unico pool che offre la configurazione del pool di storage per le app di tipo mysqldb.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* La `protection-silver-creditpoints-20k` StorageClass corrisponde al secondo pool virtuale nel `ontap-san` backend. Questo è l'unico pool che offre protezione di livello argento e 20000 punti credito.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* La `creditpoints-5k` StorageClass corrisponde al terzo pool virtuale nel `ontap-san` backend e al quarto pool virtuale nel `ontap-san-economy` backend. Queste sono le uniche offerte di pool con 5000 creditpoints.
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* La  `my-test-app-sc` StorageClass corrisponde al  `testAPP` pool virtuale nel  `ontap-san` driver con  `sanType: nvme`. Questo è l'unico pool che offre  `testApp`.
+
[source, yaml]
----
---
