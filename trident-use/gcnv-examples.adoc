...
| `snapshotDir` | Accesso a `.snapshot` directory | `true`, `false` (Il comportamento predefinito potrebbe variare. Impostare esplicitamente) "false" per NFSv3 
...
NOTE: Sostituisci  `<id_value>` e  `<key_value>` con le credenziali del tuo account di servizio.
...
[%collapsible]
====
Quando installi Trident con una cloud identity GKE, i backend si autenticano come workload identity e omettono il `apiKey` blocco. La cloud identity viene configurata allo stesso modo per entrambi i driver Google Cloud NetApp Volumes. L'unica differenza è il `storageDriverName` valore: usa `google-cloud-netapp-volumes` per i volumi NAS (NFS o SMB) e `google-cloud-netapp-volumes-san` per i volumi SAN (iSCSI block).


NOTE: Il supporto per l'identità cloud per il  `google-cloud-netapp-volumes-san` driver richiede Trident 26.06 o versioni successive. I volumi SAN vengono forniti dai pool di storage Flex Unified, quindi imposta  `serviceLevel` su  `flex`.
