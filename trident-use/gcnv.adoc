...
NOTE: Trident utilizza backend separati per i carichi di lavoro NAS e SAN in Google Cloud NetApp Volumes. Il  `google-cloud-netapp-volumes` backend supporta solo protocolli basati su file e non può essere utilizzato per il provisioning di volumi iSCSI.


NOTE: Per eseguire il provisioning dei volumi di storage a blocchi iSCSI, utilizzare il 
