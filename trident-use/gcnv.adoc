...
summary: Configura Trident per il provisioning dello storage da Google Cloud NetApp Volumes. 
---
...
Per configurare lo storage a blocchi (iSCSI), vedere xref:trident-use/gcnv-block.html[Configurare storage a blocchi (iSCSI)].
...
NOTE: Per i pool di storage UNIFIED di Google Cloud NetApp Volumes, Trident applica regole di denominazione e convalida specifiche di UNIFIED durante le operazioni sui volumi.
