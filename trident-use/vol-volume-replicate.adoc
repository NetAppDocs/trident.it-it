...
. Sul cluster Kubernetes primario, crea il TridentTridentSnapshotInfoCR per ottenere i dettagli interni.
+
.Esempio
[source, yaml]
----
...
----
+
.. Sul cluster Kubernetes secondario, aggiorna il campo _spec.state_ del CR _TridentMirrorRelationship_ a _promoted_ e _spec.volumeMappings[].promotedSnapshotHandle_ al internalName dello snapshot.
