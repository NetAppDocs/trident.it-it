...
IMPORTANT: Le modifiche influiscono su tutti i volumi che utilizzano la policy. Quando possibile, testa prima le modifiche in un ambiente non di produzione.
...
L'annotazione PVC ha sempre la precedenza. Se un PVC ha l'annotazione  `trident.netapp.io/autogrowPolicy`, Trident utilizza quel valore indipendentemente da quanto specificato in StorageClass. Consultare link:automatic-volume-expansion.html#policy-precedence["Precedenza delle policy"] per i dettagli.
